...
extends ItemSword
{
	...
	public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker)
	{
		...
		if ((target != null) && ((target instanceof EntityLivingBase)))
		{
			...
			{
				target.attackEntityFrom(DamageSourceExtra.causeAntiTitanDamage(attacker), 5000.0F);
				target.playSound("thetitans:titanpunch", 10.0F, 1.0F);
			}

			else if ((!(target instanceof EntityPlayer)) && (!(target instanceof EntityGolem)))
			{
				attacker.worldObj.removeEntity(target);
				target.playSound("random.fizz", 1.0F, 0.5F);
				attacker.addPotionEffect(new PotionEffect(Potion.field_76443_y.id, 1, 3, false));
				attacker.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(attacker.getMaxHealth() + target.getMaxHealth());
				...
				attacker.getEntityAttribute(SharedMonsterAttributes.attackDamage).setBaseValue((float)attacker.getEntityAttribute(SharedMonsterAttributes.attackDamage).getBaseValue() + (float)target.getEntityAttribute(SharedMonsterAttributes.attackDamage).getBaseValue());
			}
		}
