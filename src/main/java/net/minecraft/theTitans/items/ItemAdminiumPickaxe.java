...
extends ItemNormalPickaxe
{
	...
	public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker)
	{
		...
		if (target != null)
		{
			...
			{
				target.attackEntityFrom(DamageSourceExtra.causeAntiTitanDamage(attacker), 3000000000.0F);
	...
	public boolean onEntitySwing(EntityLivingBase entityLiving, ItemStack stack)
	{
		...
		if (!entityLiving.worldObj.isRemote && raycast != null)
		{
			...
			if (!block.getUnlocalizedName().equals("tile.corminium") && block.getBlockHardness(entityLiving.worldObj, raycast.blockX, raycast.blockY, raycast.blockZ) < Math.max(getDigSpeed(stack, block, 0) - 10.0F, 1.0F))
			{
