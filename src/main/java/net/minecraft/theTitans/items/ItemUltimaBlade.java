...
import net.minecraft.entity.titan.ITitanHitbox;
...
extends ItemTitanSword
{
	...
	public static ToolMaterial enumToolMaterialUltima = EnumHelper.addToolMaterial("Ultima", Integer.MAX_VALUE, 2, 9999999F, Float.MAX_VALUE / 2.0F, 0);
	public ItemUltimaBlade()
	{
		super("", enumToolMaterialUltima);
		soundType = 0;
		setTextureName(TheTitans.getTextures("ultima_blade"));
		setUnlocalizedName("ultima_blade");
	}
		...
		if ((list1 != null) && (!list1.isEmpty()))
		{
			for (int i1 = 0; i1 < list1.size(); i1++)
			{
				...
				if (((entity instanceof IMob || entity instanceof EntityPlayer)))
				{
					...
					{
						player.addChatMessage(new ChatComponentText("Ultima Blade: \u00A7l\u00A7oThe " + ((EntityLivingBase)entity).getCommandSenderName() +" is " + player.getDistanceToEntity(entity) + "\u00A7l\u00A7o blocks away from you."));
					}

					else
					{
						player.addChatMessage(new ChatComponentText("Ultima Blade: \u00A7l\u00A7oAn unseen mob is located out of your \u00A7l\u00A7osight " + player.getDistanceToEntity(entity) + " blocks away from you."));
						player.addChatMessage(new ChatComponentText("Ultima Blade: \u00A7l\u00A7oThe mob's health: " + ((EntityLivingBase)entity).getHealth() + "/" + ((EntityLivingBase)entity).getMaxHealth()));
						player.addChatMessage(new ChatComponentText("Ultima Blade: \u00A7l\u00A7oThe mob's name: " + ((EntityLivingBase)entity).getCommandSenderName()));
		...
		for (int l1 = -8; l1 <= 8; l1++)
		{
			for (int i2 = -8; i2 <= 8; i2++)
			{
				for (int j = -8; j <= 8; j++)
				{
					...
					{
						player.addChatMessage(new ChatComponentText("Ultima Blade: \u00A7l\u00A7oI sense a " + block.getLocalizedName() + "\u00A7l\u00A7o within atleast " + player.getDistance(j2, k, l) + " blocks of you."));
			...
			if ((list != null) && (!list.isEmpty()))
			{
				for (int i1 = 0; i1 < list.size(); i1++)
				{
					...
					if ((entity != null) && entity.isEntityAlive() && entity instanceof EntityLivingBase && !(entity instanceof EntityTitan) && entity != p_77659_3_)
					{
						...
						entity.addVelocity(-MathHelper.sin(p_77659_3_.rotationYaw * 3.1415927F / 180.0F) * 1.25F, 1D, MathHelper.cos(p_77659_3_.rotationYaw * 3.1415927F / 180.0F) * 1.25F);
	...
	public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker)
	{
		...
		if (target != null)
		{
			...
			if (((target instanceof EntityTitan)))
			{
				...
				((EntityTitan)target).hurt((40000.0F * bonusdamage) - extradamage);
				((EntityTitan)target).setAttackTarget(attacker);
				((EntityTitan)target).addTitanVelocity(-MathHelper.sin(attacker.rotationYaw * 3.1415927F / 180.0F) * 3F + knockbackAmount, 0.5D + knockbackAmount, MathHelper.cos(attacker.rotationYaw * 3.1415927F / 180.0F) * 3F + knockbackAmount);
			}
	...
	public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected)
	{
		...
		{
			((EntityPlayer)entityIn).triggerAchievement(TitansAchievments.ultimaBlade);
			if (((EntityPlayer)entityIn).inventory.hasItem(TitanItems.optimaAxe))
			{
				if ((entityIn.motionY > 1.0D) && (!entityIn.isDead))entityIn.motionY /= entityIn.motionY;
				if ((!entityIn.onGround) && (entityIn.motionY < 0.0D))
				entityIn.motionY *= 0.75D;
				if ((entityIn.motionX > 1.0D) && (!entityIn.isDead))entityIn.motionX = 1D;
				if ((entityIn.motionX < -1.0D) && (!entityIn.isDead))entityIn.motionX = -1D;
				if ((entityIn.motionZ > 1.0D) && (!entityIn.isDead))entityIn.motionZ = 1D;
				if ((entityIn.motionZ < -1.0D) && (!entityIn.isDead))entityIn.motionZ = -1D;
				if (((EntityPlayer)entityIn).isBlocking())
				{
					((EntityPlayer)entityIn).motionX = 0D;
					((EntityPlayer)entityIn).motionY = 0D;
					((EntityPlayer)entityIn).motionZ = 0D;
					((EntityPlayer)entityIn).posX = ((EntityPlayer)entityIn).lastTickPosX;
					((EntityPlayer)entityIn).posY = ((EntityPlayer)entityIn).lastTickPosY;
					((EntityPlayer)entityIn).posZ = ((EntityPlayer)entityIn).lastTickPosZ;
				}
				...
				if ((list != null) && (!list.isEmpty()))
				{
					...
					{
						Entity entity = (Entity)list.get(i1);
						if ((entity != null) && entity.isEntityAlive() && entity instanceof EntityLivingBase && ((EntityPlayer)entityIn).getRNG().nextInt(entity instanceof EntityTitan ? 10 : 100) == 0 && entity != entityIn)
						{
							if (entity != null && entity.isEntityAlive() && !(entity instanceof EntityTitanPart))
							{
								if (entity instanceof EntityPlayer)
								{
									((EntityPlayer)entity).addChatMessage(new ChatComponentText("\u00A7k" + ((EntityPlayer)entity).getRNG().nextInt(1234567890)));
								...
								EntityUrLightning lightning = new EntityUrLightning(entity.worldObj, entity.posX, entity.posY, entity.posZ);
								lightning.copyLocationAndAnglesFrom(entity);
								entity.worldObj.addWeatherEffect(lightning);
								if ((entity != null) && entity instanceof EntityLivingBase && entity.height >= 6.0F && (!(entity instanceof EntityTitan)))
								{
								...
								if (entity instanceof EntityTitan && entity instanceof ITitanHitbox)
								{
									...
									if (parts != null)
									{
										...
										{
											entity.worldObj.addWeatherEffect(new EntityUrLightning(entity.worldObj, parts[j].posX, parts[j].posY, parts[j].posZ));
										}
									}
								}
								...
								entity.motionY += 0.5D;
								entity.attackEntityFrom(new EntityDamageSource("playerwithgodpower", (EntityPlayer)entityIn).setDamageAllowedInCreativeMode().setDamageBypassesArmor().setDamageIsAbsolute(), entity instanceof EntityTitan || entity.height >= 6.0F ? 20000F : 500F);
							}
						}
					}
				}
		...
		entityIn.worldObj.spawnParticle("fireworksSpark", ((EntityLivingBase)entityIn).posX + (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5D) * ((EntityLivingBase)entityIn).width, ((EntityLivingBase)entityIn).posY + ((EntityLivingBase)entityIn).getRNG().nextDouble() * ((EntityLivingBase)entityIn).height, ((EntityLivingBase)entityIn).posZ + (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5D) * ((EntityLivingBase)entityIn).width, (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5D) * 2.0D, 1.0D, (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5D) * 2.0D);
		entityIn.worldObj.spawnParticle("explode", ((EntityLivingBase)entityIn).posX + (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5D) * ((EntityLivingBase)entityIn).width, ((EntityLivingBase)entityIn).posY + ((EntityLivingBase)entityIn).getRNG().nextDouble() * ((EntityLivingBase)entityIn).height, ((EntityLivingBase)entityIn).posZ + (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5D) * ((EntityLivingBase)entityIn).width, (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5D) * 2.0D, 1.0D, (((EntityLivingBase)entityIn).getRNG().nextDouble() - 0.5D) * 2.0D);
		if (entityIn instanceof EntityPlayer && ((EntityPlayer)entityIn).isBlocking() && !worldIn.isRemote)
		{
	...
	public boolean onEntitySwing(EntityLivingBase entityLiving, ItemStack stack)
	{
		...
		if (!entityLiving.worldObj.isRemote)
		{
			for (int i1 = 0; i1 < 24; i1++)
			{
				...
				double dy = entityLiving.getEyeHeight() + vec3.yCoord * i1;
				double dz = vec3.zCoord * i1;
				List<?> list1 = entityLiving.worldObj.getEntitiesWithinAABBExcludingEntity(entityLiving, entityLiving.boundingBox.expand(3D, 3D, 3D).offset(dx, dy, dz));
				if ((list1 != null) && (!list1.isEmpty()))
				{
					for (int i11 = 0; i11 < list1.size(); i11++)
					{
						...
						if (((entity1 instanceof EntityLivingBase)) && entity1.isEntityAlive() && (entity1 != entityLiving))
						{
							entity1.setFire(Integer.MAX_VALUE);
							entityLiving.worldObj.getGameRules().setOrCreateGameRule("keepInventory", "true");
							float extradamage = EnchantmentHelper.func_152377_a(entityLiving.getHeldItem(), ((EntityLivingBase)entity1).getCreatureAttribute());
							EnchantmentHelper.getKnockbackModifier(entityLiving, (EntityLivingBase)entity1);
							if (entity1 != null)
							{
								entity1.attackEntityFrom(DamageSourceExtra.causeAntiTitanDamage(entityLiving), 20000.0F + extradamage);
