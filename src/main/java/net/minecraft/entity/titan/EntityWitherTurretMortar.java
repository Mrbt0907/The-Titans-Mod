...
implements IRangedAttackMob
{
	...
	public void onLivingUpdate()
	{
		...
		List<?> list11 = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, getBoundingBox().expand(50.0D, 100.0D, 50.0D).offset(dx, 0D, dz));
		if ((list11 != null) && (!list11.isEmpty()) && (getAttackTarget() == null))
		{
			for (int i1 = 0; i1 < list11.size(); i1++)
			{
				Entity entity = (Entity)list11.get(i1);
				if (((entity instanceof EntityLivingBase)) && (entity != null) && (entity.isEntityAlive()) && (getAttackTarget() == null) && (canTargetEntity((EntityLivingBase)entity)) && (canEntityBeSeen(entity)))
				{
		...
		if (getAttackTarget() != null)
		{
			...
			getLookHelper().setLookPositionWithEntity(getAttackTarget(), 10.0F, 180.0F);
			this.renderYawOffset = this.rotationYaw = this.rotationYawHead;
			if ((!getAttackTarget().isEntityAlive()) || (getAttackTarget().isDead) || (getAttackTarget().getDistanceSqToEntity(this) > 10000.0D) || (!canEntityBeSeen(getAttackTarget())))
			{
	...
	{
		launchWitherSkullToCoords(p_82216_1_, p_82216_2_.posX, p_82216_2_.posY + 0.25D, p_82216_2_.posZ, false);
	}
	...
	private void launchWitherSkullToCoords(int p_82209_1_, double p_82209_2_, double p_82209_4_, double p_82209_6_, boolean p_82209_8_)
	{
		if (this.shootingTimer <= 0)
		{
			...
			playSound("thetitans:mortarShoot", 3.0F, 1.0F);
			...
			EntityMortarWitherSkull entitywitherskull = new EntityMortarWitherSkull(this.worldObj, this, d6, d7, d8);
	...
	protected void onDeathUpdate()
	{
		...
		if ((!this.worldObj.isRemote) && (this.worldObj.getGameRules().getGameRuleBooleanValue("doMobLoot") == true))
		{
			...
			{
				entityDropItem(new ItemStack(Items.skull, 1, 1), 10.0F);
				entityDropItem(new ItemStack(Items.skull, 1, 1), 9.0F);
				entityDropItem(new ItemStack(Items.skull, 1, 1), 8.0F);
				entityDropItem(new ItemStack(Blocks.beacon, 1), 6.0F);
				dropItem(Items.nether_star, this.rand.nextInt(3));
			}
			...
			while (i > 0)
			{
				...
				i -= j;
				this.worldObj.spawnEntityInWorld(new EntityXPOrb(this.worldObj, this.posX, this.posY + 9.0D, this.posZ, j));
			}
		...
		if (!this.worldObj.isRemote)
		{
			playSound("thetitans:turretDeath3", 10.0F, 1.0F);
			this.worldObj.createExplosion((Entity)null, this.posX, this.posY - 1.0D, this.posZ, 3.0F, true);
		}
	}
