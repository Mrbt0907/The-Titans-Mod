...
import net.mrbt0907.thetitans.entity.EntityGammaLightning;

public class RenderGammaLightning extends RenderLightningBolt
{
	public RenderGammaLightning(RenderManager renderManagerIn)
	{
	...
	{
		if (!(entity instanceof EntityGammaLightning))
			return;
		EntityGammaLightning lightning = (EntityGammaLightning) entity;
            ...
            for (int j = 0; j < 3; ++j)
            {
                ...
                for (int i1 = k; i1 >= l; --i1)
                {
                    ...
                    for (int j1 = 0; j1 < 5; ++j1)
                    {
                        ...
                        float[] color = lightning.getColor();
                        bufferbuilder.pos(d10 + d2, y + (double)(i1 * 16), d11 + d3).color(color[0], color[1], color[2], 0.45F).endVertex();
                        bufferbuilder.pos(d8 + d4, y + (double)((i1 + 1) * 16), d9 + d5).color(color[0], color[1], color[2], 0.45F).endVertex();
                    }
