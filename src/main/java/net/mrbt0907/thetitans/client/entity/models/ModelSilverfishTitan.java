...
public class ModelSilverfishTitan extends ModelBase
{
    ...
    {
    	EntitySilverfishTitanProto titan = (EntitySilverfishTitanProto)entityIn;
    	
    	float f = 0.9F - (titan.getSizeMultiplier() * 0.0125F);
    	
        for (int i = 0; i < this.silverfishBodyParts.length; ++i)
        {
            this.silverfishBodyParts[i].rotateAngleY = MathHelper.cos(ageInTicks * f + (float)i * 0.15F * (float)Math.PI) * (float)Math.PI * 0.05F * (float)(1 + Math.abs(i - 2));
            this.silverfishBodyParts[i].rotationPointX = MathHelper.sin(ageInTicks * f + (float)i * 0.15F * (float)Math.PI) * (float)Math.PI * 0.2F * (float)Math.abs(i - 2);
        }
