...
public class EntityCaveSpiderTitanProto extends EntityPreTitan
{
	...
	{
		playSound(this.getSizeMultiplier() <= 7 || this.ticksExisted <= 1 ? (this.getSizeMultiplier()  > 2 ? SoundEvents.ENTITY_SPIDER_STEP : SoundEvents.ENTITY_SPIDER_STEP) : SoundRegistry.get("titan.step"), this.getSoundVolume(), this.getSizeMultiplier() <= 7 || this.ticksExisted <= 1 ? this.getSoundPitch() : getSoundPitch() + 1.0F);
	}
	...
	{
		double hp = 4000D * this.getSizeMultiplier();
